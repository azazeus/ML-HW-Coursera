...
Y= y * ones(1,num_labels); % 5000 x 10
boolY = ones(m,1)*(1:1:num_labels); % 5000 x 10
boolY=(boolY==y); % 5000 x 10
J=-1/m*sum(sum(log(a3').*boolY + (1-boolY).*log(1-a3'))) + ...
...
for s = 1:m
	delta3(:,s) = (a3(:,s)-boolY(s,:)'); % 10 x 5000
	delta2(:,s) = (Theta2(:,2:end)'*delta3(:,s)).*sigmoidGradient(Theta1*X(s,:)'); % 25 x 5000
	D2 = D2 + delta3(:,s)*a2(:,s)'; % 10 x 26
	D1 = D1 + delta2(:,s)*X(s,:); % 25 x 401
end
